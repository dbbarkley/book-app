// User Search Hook - Search users with debounce and pagination
...
/**
 * Hook for searching users with debounce and pagination
 * 
 * Usage:
 * ```tsx
 * const { users, loading, search, loadMore, hasMore } = useUserSearch({
 *   debounceMs: 300,
 *   pageSize: 20
 * })
 * 
...
export function useUserSearch(options: UseUserSearchOptions = {}) {
  ...
  const performSearch = useCallback(
    async (searchQuery: string, page: number = 1, append: boolean = false) => {
      ...
      try {
        const result = await apiClient.searchUsers(searchQuery, page, pageSize)
