...
import type { UserBook, ShelfStatus } from '@book-app/shared/types'
...
const SHELF_CONFIG: Record<string, { label: string; subtitle: string; color: string }> = {
  reading: {
    label:    'Reading',
    subtitle: 'Books you\'re currently working through',
  ...
  read: {
    label:    'Completed',
    subtitle: 'Every book you\'ve finished',
  ...
  private: {
    label:    'Private',
    subtitle: 'Books only you can see',
...
export default function ShelfPage({ params }: { params: { shelf: string } }) {
  const { shelf } = params
  ...
  useEffect(() => {
    if (!config) { router.replace('/library'); return }
    if (!isAuthenticated) return
    setLoading(true)
    const params = shelf === 'private'
      ? { visibility: 'private' as const }
      : { shelf: shelf as ShelfStatus }
    apiClient.getUserBooks(params)
      .then(({ user_books }) => setBooks(user_books))
      .catch(console.error)
      .finally(() => setLoading(false))
  }, [shelf, isAuthenticated, config, router])
  ...
  return (
    <>
                ...
                >
                  {SORT_OPTIONS.map((opt, i) => (
                    <button
                      ...
                      style={{
                        color: sortKey === opt.key ? config.color : 'var(--color-ink)',
          ...
          {!loading && books.length === 0 && (
            ...
            >
              ...
              <p className="text-[14px]" style={{ color: 'var(--color-ink-3)' }}>
                Add books to your {config.label.toLowerCase()} shelf and they'll show up here.
              </p>
            </div>
          )}
                ...
                return (
                  <div key={userBook.id} className="group/card">
                    ...
                    <div className="relative mb-3">
                      ...
                      {/* Rating badge (completed shelf) */}
                      {shelf === 'read' && userBook.rating && (
                        <div
                      ...
                      {/* Progress badge (reading shelf) */}
                      {shelf === 'reading' && userBook.completion_percentage != null && userBook.completion_percentage > 0 && (
                        <div
                          className="absolute bottom-2 left-2 text-[11px] font-bold"
                          style={{
                            backgroundColor: 'var(--color-canvas)',
      ...
      <QuickUpdateModal
        userBook={modalBook || { id: 0, book_id: 0, status: shelf as ShelfStatus } as any}
        isOpen={!!modalBook}
